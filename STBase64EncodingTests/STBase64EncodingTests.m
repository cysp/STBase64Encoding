//
//  STBase64EncodingTests.m
//  STBase64EncodingTests
//

#import "STBase64EncodingTests.h"

#import "STBase64Encoding.h"


@implementation STBase64EncodingTests {
	NSMutableArray *_testcases;
	NSMutableArray *_testcasesDecodeError;
}

- (void)setUp {
	struct testcase {
		void *decodedBytes;
		unsigned long decodedLength;
		void *encodedBytes;
		unsigned long encodedLength;
	} testcases[] = {
		{
			"", 0, "", 0,
		},
		{
			"a", 1, "YQ==", 4,
		},
		{
			"aa", 2, "YWE=", 4,
		},
		{
			"aaa", 3, "YWFh", 4,
		},
		{
			"aaaa", 4, "YWFhYQ==", 8,
		},
		{
			"A", 1, "QQ==", 4,
		},
		{
			"AB", 2, "QUI=", 4,
		},
		{
			"ABC", 3, "QUJD", 4,
		},
		{
			"ABCD", 4, "QUJDRA==", 8,
		},
		{
			"f", 1, "Zg==", 4,
		},
		{
			"fo", 2, "Zm8=", 4,
		},
		{
			"foo", 3, "Zm9v", 4,
		},
		{
			"foob", 4, "Zm9vYg==", 8,
		},
		{
			"fooba", 5, "Zm9vYmE=", 8,
		},
		{
			"foobar", 6, "Zm9vYmFy", 8,
		},
	};
	_testcases = [NSMutableArray array];

	for (int i = 0; i < sizeof(testcases) / sizeof(testcases[0]); ++i) {
		struct testcase testcase = testcases[i];
		[_testcases addObject:@[ [NSData dataWithBytes:testcase.decodedBytes length:testcase.decodedLength], [NSData dataWithBytes:testcase.encodedBytes length:testcase.encodedLength] ] ];
	}

	{
		unsigned char decodedBytes[256] = {  };
		NSUInteger decodedLength = 256;
		char *encodedBytes = "";
		NSUInteger encodedLength = 344;
		[_testcases addObject:@[ [NSData dataWithBytes:decodedBytes length:decodedLength], [NSData dataWithBytes:encodedBytes length:encodedLength] ] ];
	}
}

- (void)testEncoding {
	for (NSArray *testcase in _testcases) {
		NSData *input = testcase[0];
		NSData *expected = testcase[1];

		NSData *output = [STBase64Encoding dataByBase64EncodingData:input];
		STAssertEqualObjects(output, expected, @"");
	}
}

- (void)testDecoding {
	for (NSArray *testcase in _testcases) {
		NSData *input = testcase[1];
		NSData *expected = testcase[0];

		NSError *error = nil;
		NSData *output = [STBase64Encoding dataByBase64DecodingData:input withOptions:0 error:&error];
		STAssertNotNil(output, @"err: %@", error);
		STAssertEqualObjects(output, expected, @"");
	}

	struct testcase {
		void *decodedBytes;
		unsigned long decodedLength;
		void *encodedBytes;
		unsigned long encodedLength;
	};

	struct testcase testcases[] = {
		{
			"", 0, "====", 4,
		},
		{
			"a", 1, "YQ======", 8,
		},
	};
	for (int i = 0; i < sizeof(testcases) / sizeof(testcases[0]); ++i) {
		struct testcase testcase = testcases[i];
		NSData *input = [NSData dataWithBytesNoCopy:testcase.encodedBytes length:testcase.encodedLength freeWhenDone:NO];
		NSData *expected = [NSData dataWithBytesNoCopy:testcase.decodedBytes length:testcase.decodedLength freeWhenDone:NO];

		NSError *error = nil;
		NSData *output = [STBase64Encoding dataByBase64DecodingData:input withOptions:0 error:&error];
		STAssertNotNil(output, @"err: %@", error);
		STAssertEqualObjects(output, expected, @"");
	}

	struct testcase sloppycases[] = {
		{
			"", 0, "== ==", 5,
		},
		{
			"a", 1, "Y Q======", 9,
		},
		{
			"a", 1, "Y\rQ===\n=== ", 11,
		},
	};
	for (int i = 0; i < sizeof(sloppycases) / sizeof(sloppycases[0]); ++i) {
		struct testcase sloppycase = sloppycases[i];
		NSData *input = [NSData dataWithBytesNoCopy:sloppycase.encodedBytes length:sloppycase.encodedLength freeWhenDone:NO];
		NSData *expected = [NSData dataWithBytesNoCopy:sloppycase.decodedBytes length:sloppycase.decodedLength freeWhenDone:NO];

		NSError *error = nil;
		NSData *output = [STBase64Encoding dataByBase64DecodingData:input withOptions:STBase64DecodingOptionSkipInvalidInputBytes error:&error];
		STAssertNotNil(output, @"err: %@", error);
		STAssertEqualObjects(output, expected, @"");
	}

	struct errorcase {
		void *encodedBytes;
		unsigned long encodedLength;
	} errorcases[] = {
		{
			"QUJDRA==========QUJD", 20,
		},
		{
			"YQ==YQ==", 8,
		},
		{
			"= ===", 5,
		},
	};

	for (int i = 0; i < sizeof(errorcases) / sizeof(errorcases[0]); ++i) {
		struct errorcase errorcase = errorcases[i];
		NSData *input = [NSData dataWithBytesNoCopy:errorcase.encodedBytes length:errorcase.encodedLength freeWhenDone:NO];

		NSError *error = nil;
		NSData *output = [STBase64Encoding dataByBase64DecodingData:input withOptions:0 error:&error];
		STAssertNil(output, @"i: %@ o: %@", input, output);
		STAssertNotNil(error, @"");
	}
}

@end
